    ...
    return (
        <div className="flex flex-col gap-2 rounded-2xl border border-rule bg-surface p-5">
            ...
            <div className="flex items-end justify-between gap-2">
                ...
                {change && change.value !== "–" && (
                    <span
                        className={`label rounded-full px-2 py-0.5 ${up ? "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400"
                ...
                {items.map((item) => (
                    <div key={item.label} className="flex items-center gap-3 text-sm">
                        <div className="relative h-6 flex-1 overflow-hidden rounded-sm bg-rule/50">
                            <div
                                className="absolute left-0 top-0 h-full rounded-sm bg-signal/20 transition-all duration-500"
...
export function LineChart({ data, width = 600, height = 200 }: LineChartProps) {
    ...
    return (
        <div className="w-full overflow-hidden">
            ...
            {/* Screen-reader table */}
            <table className="sr-only">
                <caption>Views and visitors over time</caption>
                <thead>
                    <tr>
                        <th>Date</th>
                        <th>Views</th>
            ...
            >
                <g transform={`translate(${pad.left},${pad.top})`}>
                    ...
                    {yTicks.map((tick) => (
                        <g key={tick}>
                            ...
                            <text x={-6} y={yOf(tick)} textAnchor="end" dominantBaseline="middle"
                                fill="rgb(var(--muted))" fontSize="10">
                    ...
                    {xLabels.map((d) => {
                        ...
                        return (
                            <text key={d.t} x={xOf(i)} y={innerH + 14}
                                textAnchor="middle" fill="rgb(var(--muted))" fontSize="10">
                                {d.t.slice(5)}
                            </text>
                        );
                    })}
                    ...
                    return (
                        <div key={step.label} className="flex flex-col gap-1">
                            ...
                            <div className="h-8 relative w-full overflow-hidden rounded bg-rule/40">
                                <div
                                    className="absolute left-0 top-0 h-full rounded bg-signal/70 transition-all duration-700"
                                    style={{ width: `${pct}%` }}
                                />
                            </div>
                            {i > 0 && dropoff > 0.1 && (
